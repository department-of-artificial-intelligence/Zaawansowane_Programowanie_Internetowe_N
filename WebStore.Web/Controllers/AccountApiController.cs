...
namespace WebStore.Web.Controllers
{
    public class AccountApiController : BaseApiController
    {
        ...
        public AccountApiController(ILogger logger, IMapper mapper, IOptions<JwtOptionsVm> jwtOptions, SignInManager<User> signInManager, UserManager<User> userManager) : base(logger, mapper)
        {
        ...
        public async Task<IActionResult> Login([FromForm] LoginUserVm applicationUser)
        {
            try
            {
                if (applicationUser == null || ModelState == null || !ModelState.IsValid)
                {
                ...
                var claims = new List<Claim>
                {
                    ...
                    new Claim(JwtRegisteredClaimNames.Exp, ((long)((DateTime.Now.AddMinutes(_jwtOptions.TokenExpirationMinutes) - new DateTime(1970, 1, 1, 0, 0, 0)).TotalSeconds)).ToString())
                };

                claims.AddRange(userRoles.Select(ur => new Claim(ClaimTypes.Role, ur)));

                var token = new JwtSecurityToken(
                    new JwtHeader(new SigningCredentials(new SymmetricSecurityKey(Encoding.UTF8.GetBytes(_jwtOptions.SecretKey)), SecurityAlgorithms.HmacSha256)),
